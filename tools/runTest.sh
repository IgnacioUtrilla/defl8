...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
  fswatch -o --event Updated --event OwnerModified "$DIR_WATCH" -Ie".*\.${tempFilename}$" | while read event; do
    # 5 = Updated, 2 = OwnerModified
    if [ $event -eq 5 ] || [ $event -eq 2 ]; then
      compileAndRunTest
    fi
  done
elif [[ "$OSTYPE" == "darwin"* ]]; then
  fswatch -o "$DIR_WATCH" -Ie".*\.${tempFilename}$" | while read event; do
    # 2 = Updated
    if [ $event -eq 2 ]; then
      compileAndRunTest
    fi
